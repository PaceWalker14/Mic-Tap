...
Distance moves add one more layer: every tap's peak loudness is kept with its group, and when a two-tap group lands, the two peaks are compared. If the first tap is louder than the second by at least the **Close / Far distance** gap (Medium = 8 dB), you moved away — **Close · Far**; that much quieter and you moved in — **Far · Close**; anything in between stays a plain double tap. It rides on loudness (a single mic's only distance cue), so tap with even force and vary only the distance — the radar shows what each tap read as, and while a distance move is bound the log prints a "Distance read" line for every two-tap group telling you the measured gap versus what's needed. Patterns with no action bound don't divert: an uneven double tap still counts as a double unless you've assigned the matching distance move.
...
`python test_detector.py` feeds synthetic audio through the detector, calibrator, and reject-learner: it checks that double/triple/single taps and both distance moves are recognized, that clicky "keystrokes" (which also thump the desk) are rejected by the tap fingerprint, that slow-onset sounds are rejected by the shape gate, that clipped/outlier calibration taps are dropped, and that a learned ignore-sound drops them with the right reason. Useful for checking logic changes without tapping at your desk like a woodpecker.
